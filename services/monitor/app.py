"""
Monitor Service — System dashboard with host metrics, GPU, and container stats.
Delegates Docker/LLM/session data to core-api.
"""
...
async def status():
    ...
    async def fetch_core(path):
        async with httpx.AsyncClient(base_url=CORE_API, timeout=30.0) as c:
            r = await c.get(path)
            return r.json()

    MICROSERVICE_HEALTH_ENDPOINTS = {
        "core-api":   "http://core-api:8000/health",
        "monitor":    "http://monitor:9091/api/health",
        "heartbeat":  "http://heartbeat:9092/api/health",
        "calendar":   "http://calendar:9093/api/health",
        "chat":       "http://chat:9094/api/health",
        "landing":    "http://landing:9095/api/health",
        "finance":    "http://finance:9096/api/health",
        "nutrition":  "http://nutrition:9097/api/health",
    }
    ...
    health_checks = await asyncio.gather(
        *[check_service_health(n, u) for n, u in MICROSERVICE_HEALTH_ENDPOINTS.items()]
    )
    service_health = {h["name"]: h["ok"] for h in health_checks}
    ...
    if openclaw_ok and not has_errors and services_ok:
        overall = "healthy"
    elif openclaw_ok:
        overall = "degraded"
    else:
