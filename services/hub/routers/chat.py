"""
Chat Router — LLM conversation interface (APIRouter for the hub monolith).
Delegates to core-api routes on localhost for skills, sessions, and system-prompt.
...
# ── Client lifecycle (called by main app lifespan) ───────────────────────────

async def init_clients():
    """Create httpx clients. Must be called during app startup."""
...
async def close_clients():
    """Close httpx clients. Must be called during app shutdown."""
    global _http_client, _coding_client, _core_client
    if _http_client:
...
async def skills():
    """Proxy skills from core-api (localhost)."""
    try:
...
async def sessions_list():
    """Proxy sessions list from core-api (localhost)."""
    try:
...
async def session_messages(session_id: str):
    """Proxy session messages from core-api (localhost)."""
    try:
...
async def delete_session(session_id: str):
    """Proxy session deletion to core-api (localhost)."""
    try:
...
async def save_session(request: dict):
    """Proxy session save to core-api (localhost)."""
    try:
...
@router.get("/api/supported-files")
